...
char* handler_joingroup(int sock, char* opt)
{
    ...
    getpeername(sock, (struct sockaddr*)gruppo_giocatori.indirizzi[1], &len);
...
static const comando lista_comandi_client[] = {
    ...
    {"joingroup", handler_joingroup, "\tentra nel gruppo se è stato creato"},
...
{
    int i;
    char* res = "Lista comandi disponibili:\n";
    char tmp[1024];
    for(i = 0; i < N_COMANDI_CLIENT; i++) {
        sprintf(tmp, "\t%s%s\n", lista_comandi_client->nome, lista_comandi_client->descrizione);
        strcat(res, tmp);
    }
    return res;
}
